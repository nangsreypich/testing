...
if(isset($_POST["do"])&&($_POST["do"]=="add_events")){
	...
	$sql="INSERT INTO events (title,note,color,category_id,grade_id,create_by,creator_type,google_link,start_date_time,end_date_time,year,month) 
     	  VALUES ( '".$title."','".$note."','".$color."','".$category_id."','".$grade_id."','".$my_index."','".$my_type."', '".$google_link."','".$start_date_time1."','".$end_date_time1."','".$year."','".$month."')";
		...
		if(mysqli_num_rows($result4) > 0){
			while($row4=mysqli_fetch_assoc($result4)){
				...
				$sql5="INSERT INTO notification_history (notification_id,index_number,user_type,_isread) 
					  VALUES ( '".$last_id."','".$index_number4."','Staff',0)";
		...
		if(isset($_POST["checkbox"])){
			for($i=0;$i<count(($_POST["checkbox"]));$i++){
				...
				$sql="select student.index_number as std_index 
					  ...
					  on student.index_number=student_course.index_number
					  where student_course.year='$year' and student_course.grade_id='$grade1'";
